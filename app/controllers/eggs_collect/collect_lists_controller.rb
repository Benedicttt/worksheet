        ...
        wb = @p.workbook
          ...
          EggCollect.all.order(month: :asc).where(year: params[:year]).map(&:month).compact.uniq.each do |month|
            ...
            content_style_left_text = wb.styles.add_style  :b => true, :sz => 8,  :font_name => 'Monaco', :alignment => { :horizontal => :right, :vertical => :center, :wrap_text => true}
              ...
              if !period.nil?
                deads_chick = EggCollect.where(period: period_id, house: params[:house], created_at: ..(DateTime.new(params[:year].to_i, month.to_i, 1))).map(&:deads_chick).inject(0){ |sum, x| sum + x }
                deads_hen = EggCollect.where(period: period_id, house: params[:house], created_at: ..(DateTime.new(params[:year].to_i, month.to_i, 1))).map(&:deads_hen).inject(0){ |sum, x| sum + x }
